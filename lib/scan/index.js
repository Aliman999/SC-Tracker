...
/*
        ...
        if(data.length){
          if(data[0] == player){ 
            //We can check to see if the same user is being called meaning we're at the end of our list
            ...
            organizations.forEach(org => {
              db.query({ "data.sid": org.sid }, "organizations").then(data => {
                if(data.length){
                  //We have the organization in the database and in the future we will check history
                  count++
                }else{
                  //We do not have the organization in the databse and we store the organization.
                  api.organization.members.all(org.sid).then(org => {
                    count++;
                    //warehouse.organization(org);
                  })
